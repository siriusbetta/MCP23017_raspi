...
void MCP23017::setI2CBus_Addr(int bus, int address)
{
    //printf("MCP23017 input bus num: %d\n", bus);
    kI2CBus = bus;           // I2C bus of Jetson (1 and 8 available on Xavier)
    //printf("MCP23017 bus num: %d\n", kI2CBus);
    kI2CAddress = address ; // Address of MCP23017; defaults to 0x20
    //printf("MCP23017 addr : %x\n", kI2CAddress);
    error = 0 ;
}

//helper function to replace bitRead function from Arduino library
bool MCP23017::bitRead(int32_t num, uint8_t index)
{
    return (num >> index) & 1;
}
...
bool MCP23017::openI2C()
{
    ...
    if (kI2CFileDescriptor < 0) {
        std::cerr << "Could not open the file: " << fileNameBuffer << std::endl;
       error = errno ;
       return false ;
    }
    if (ioctl(kI2CFileDescriptor, I2C_SLAVE, kI2CAddress) < 0) {
        std::cerr << "Could not open the bus" << std::endl;
